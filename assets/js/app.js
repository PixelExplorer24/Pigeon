        ...
        window.openFriendChat = async function(uid) {
            ...
            const render=async()=>{
                ...
                box.innerHTML=messages.map(m=>{const mine=m.senderUid===currentUser.uid; return `<div class="flex ${mine?'justify-end':'justify-start'}"><div class="group max-w-[82%] px-3 py-2 rounded-2xl ${mine?'bg-wax-red text-white rounded-br-md':'bg-parchment-200 text-parchment-900 rounded-bl-md'}"><div class="text-xs whitespace-pre-wrap break-words">${escapeHtmlSafe(m.text)}</div><div class="flex items-center ${mine?'justify-end':'justify-start'} gap-2 mt-1"><span class="text-[9px] opacity-70">${escapeHtmlSafe(formatMessageTime(m.createdAt))}</span>${mine?`<button type="button" onclick="deleteDirectMessage('${m.id}')" class="text-[9px] opacity-70 hover:opacity-100 underline">Delete</button>`:''}</div></div></div>`}).join('')||'<div class="text-xs text-center text-parchment-600 py-8">No messages yet. Start the private conversation.</div>';
