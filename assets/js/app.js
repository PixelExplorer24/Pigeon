...
async function acceptRequest(id,uid){
  ...
  try{
    ...
    const otherSnap=await USERS().doc(uid).get();
    const other=otherSnap.exists?(otherSnap.data()||{}):{};
    const mine={displayName:profile?.displayName||me.displayName||me.email?.split("@")[0]||"User",photoURL:profile?.photoURL||me.photoURL||null,email:profile?.email||me.email||"",bio:profile?.bio||""};
    const otherProfile={displayName:other.displayName||"User",photoURL:other.photoURL||null,email:other.email||"",bio:other.bio||""};
    const batch=db.batch();
    batch.set(FRIENDS().doc(p+"__"+me.uid),{pairId:p,ownerUid:me.uid,friendUid:uid,requestId:id,...otherProfile,createdAt:now},{merge:true});
    batch.set(FRIENDS().doc(p+"__"+uid),{pairId:p,ownerUid:uid,friendUid:me.uid,requestId:id,...mine,createdAt:now},{merge:true});
